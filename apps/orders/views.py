...
async def show_orders(session: AsyncSession = Depends(vortex.scoped_session_dependency)):
    return await get_orders(session=session)


@router.post("/add_order")
async def add_order(promocode: str | None,
...
async def del_order(order_id: int, session: AsyncSession = Depends(vortex.scoped_session_dependency)):
