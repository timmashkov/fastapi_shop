...
async def take_products(session: AsyncSession = Depends(db_helper.scoped_session_dependency)):
    return await get_products(session=session)


@router.post('/add', response_model=Product)
async def add_product(product_in: ProductCreate,
                      session: AsyncSession = Depends(db_helper.scoped_session_dependency)):
    return await create_product(session=session, data=product_in)


@router.get('/{id}/', response_model=Product)
async def take_product(id: int,
                       session: AsyncSession = Depends(db_helper.scoped_session_dependency)):
...
@router.put('/{id_upd}')
async def upd_product(product_upd: ProductUpdatePartial,
                      session: AsyncSession = Depends(db_helper.scoped_session_dependency),
                      product: Product = Depends(product_by_id)):
    return await update_product(session=session,
                                product=product,
                                data=product_upd,
                                partial=True)
...
@router.delete('/{id}')
async def del_product(product: Product = Depends(product_by_id),
                      session: AsyncSession = Depends(db_helper.scoped_session_dependency)):
