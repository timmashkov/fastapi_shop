...
@router.get(
    "/show_users", response_model=list[UserResponseSchema], description="Show all users"
)
# @cache(expire=60)
...
@router.get("/show_user/{username}", description="Show specific user")
# @cache(expire=60)
async def show_user(
...
@router.get("/user_profile", description="Show user with profile")
@cache(expire=60)
...
@router.get("/user_post", description="Show user with posts")
@cache(expire=60)
...
@router.put(
    "/update_user", response_model=UserUpdatePartial, description="Change user info"
)
...
@router.delete("/delete_user/{user}", description="Delete user")
